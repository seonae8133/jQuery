//item.js
$(document).ready(function () { //$ : jquery선택자를 가져오는 방식
...
function createContentItem(item) {
    ...
    let a1 = $('<a />').attr('href', item.link + '?item_no=' + item.item_no); //data.js에 있는 link부분의 값을 읽어와서a태그의 속성값으로 넣어줌
    ...
    let star = '';
    let cnt = Math.floor(item.star); //floor : 바닥값
    for (let i = 0; i < cnt; i++) {
        star += '&#9733;'
    }
